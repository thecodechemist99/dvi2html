...
function processTfmFile( fontname ) {
  console.log( fontname );

  const data =  tfm.loadFont(fontname);
  const characters = {}
  for( const k of Object.keys(data.characters)){
    const item = data.characters[k]
    characters[k] = {
    width: item.width,
    height: item.height,
    depth: item.depth
  ...
  fonts[fontname] = {
    characters,
    smallest_character_code: data.smallest_character_code,
    largest_character_code: data.largest_character_code,
...
desiredFonts.forEach(processTfmFile);

var outputFile = fs.openSync( outputPath, 'w' );
console.log(fonts)
fs.writeFileSync( outputFile, Buffer.from(JSON.stringify(fonts), 'utf8') );
